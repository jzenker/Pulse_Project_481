...
//Arduino Mega: Sender 
//This code changes, updates and stores information about the current color of 
//an LED strip and simulated heart rate value. This information is then sent 
//over serial communication a the NodeMCU. 
//Finite state machine code adapted from Grumpy Mike: https://forum.arduino.cc/index.php?topic=412232.0
...
//How to communicate from Arduino to NodeMCU
...
void loop() {
  ...
  if (currentMillis - previousButtonMillis_2 > sendInterval) {
    ...
    ArduinoUno.println(result); //Prints data oversoftware serial 
...
//Used as a placeholder for the heart rate due to the sensor not working properly 
...
//Each time the button is pressed, it will change to the next color 
...
/*
 * Color functions below are modified from the Adafruit Neopixel library 
*/
